...
imsize48_architecture_2 = dict(
    ...
    deconv_args=dict(
        ...
        deconv_input_width=12,
        deconv_input_height=12,
        deconv_input_channels=32,

        deconv_output_kernel_size=3,
        deconv_output_strides=2,
        deconv_output_channels=3,
        deconv_output_padding=1,
        deconv_output_output_padding=1,
...
class ConvVAE(GaussianLatentVAE):
    def __init__(
        ...
        if 'output_size' not in conv_kwargs:
            conv_kwargs['output_size'] = (deconv_args['deconv_input_width']
                                          * deconv_args['deconv_input_height']
                                          * deconv_args['deconv_input_channels'])

        if 'paddings' not in conv_args:
            conv_args['paddings'] = np.zeros(len(conv_args['kernel_sizes']),
                                             dtype=np.int64)
        ...
        if 'paddings' not in deconv_args:
            deconv_args['paddings'] = np.zeros(len(deconv_args['kernel_sizes']),
                                               dtype=np.int64)
        if 'output_paddings' not in deconv_args:
            deconv_args['output_paddings'] = np.zeros(len(deconv_args['kernel_sizes']),
                                                      dtype=np.int64)
    ...
    def logprob(self, inputs, obs_distribution_params):
        ...
        if self.decoder_distribution == 'gaussian_identity_variance':
            inputs = inputs.narrow(start=0, length=self.imlength,
                                   dim=1).contiguous().view(-1, self.imlength)
            log_prob = -1 * F.mse_loss(obs_distribution_params[0], inputs,
                                       reduction=self.mse_reduction)
            if self.normalize_logprob:
                log_prob = log_prob / inputs.shape[0]
            return log_prob
        else:
            raise NotImplementedError('Distribution {} not supported'.format(
                self.decoder_distribution))
