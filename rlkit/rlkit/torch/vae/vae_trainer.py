...
# from rlkit.torch.vae.evaluation import compute_mig
...
class ConvVAETrainer(object):
    def __init__(
        ...
        self.img_dump_interval = img_dump_interval
        ...
        for batch_idx in range(batches):
            ...
            if self.log_interval and batch_idx % self.log_interval == 0:
                logger.log('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                    ...
                    batch_idx * len(data),
                    len(data) * batches,
                    100. * batch_idx / batches,
                    loss.item() / len(next_obs)))

            if batch_idx == batches - 1 and epoch % self.img_dump_interval == 0:
                n = min(next_obs.size(0), 8)
                comparison = torch.cat([
                    next_obs[:n].narrow(start=0, length=self.imlength, dim=1)
                                .contiguous()
                                .view(-1, self.input_channels,
                                      self.imsize, self.imsize)
                                .transpose(2, 3).flip(2),
                    reconstructions.view(self.batch_size, self.input_channels,
                                         self.imsize, self.imsize)[:n]
                                   .transpose(2, 3).flip(2)
                ])
