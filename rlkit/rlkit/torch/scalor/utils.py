...
def visualize_one_image(x, z_where):
    """
        x: (1, 3, img_h, img_w)
        z_where: (n_objects, 4)
    """
    img_h, img_w = x.shape[2], x.shape[3] 
    n_objects = z_where.size(0)
    z_pres = torch.ones(n_objects, 1, 1, 1)
    bbox = spatial_transform(z_pres * gbox,
                             z_where,
                             torch.Size([n_objects, 3, img_h, img_w]),
                             inverse=True)
    bbox = bbox.view(1, -1, 3, img_h, img_w)
    bbox = (bbox.sum(dim=1) + x.cpu()).clamp(0, 1)
    return bbox
...
def print_scalor(global_step, total_loss, log_like, z_what_kl_loss, z_where_kl_loss,
                 z_pres_kl_loss, z_depth_kl_loss):
