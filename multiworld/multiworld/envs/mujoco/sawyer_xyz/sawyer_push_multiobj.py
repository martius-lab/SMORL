...
class SawyerMultiobjectEnv(MujocoEnv, Serializable, MultitaskEnv):
    ...
    def __init__(
        ...
        self.obj_stat_prop = create_object_xml(base_filename, num_objects, object_mass,
                                               ...
                                               block_height, block_width, cylinder_radius, use_textures)
...
class SawyerTreeObjectEnv(SawyerMultiobjectEnv):
    """
    This environment matches exactly the 3-object pushing environment in the RIG paper
    """
    PUCK1_GOAL_LOW = np.array([0.0, 0.5])
    PUCK1_GOAL_HIGH = np.array([0.2, 0.7])
    PUCK2_GOAL_LOW = np.array([-0.2, 0.5])
    PUCK2_GOAL_HIGH = np.array([0.0, 0.7])
    PUCK3_GOAL_LOW = np.array([-0.2, 0.3])
    PUCK3_GOAL_HIGH = np.array([0.0, 0.5])
    HAND_GOAL_LOW = np.array([-0.05, 0.55])
    HAND_GOAL_HIGH = np.array([0.05, 0.65])

    low = np.hstack((HAND_GOAL_LOW, PUCK1_GOAL_LOW, PUCK2_GOAL_LOW, PUCK3_GOAL_LOW))
    high = np.hstack((HAND_GOAL_HIGH, PUCK1_GOAL_HIGH, PUCK2_GOAL_HIGH, PUCK3_GOAL_HIGH))
    ...
    def sample_goal_for_rollout(self):
        if self.randomize_goals:
            ...
            pos = self.FIXED_GOAL_INIT.copy()
        return np.hstack((hand, g1, g2, g3))
